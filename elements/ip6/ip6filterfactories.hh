#ifndef CLICK_IP6FILTER_FACTORIES_HH
#define CLICK_IP6FILTER_FACTORIES_HH
...
#include "ip6filtertokens_ethernet.hh"
...
public:
    static Token* create_token(String argument_of_ip6_flowlabel_keyword, bool just_seen_a_not_keyword, Operator an_operator) {
        ...
        return NULL;    // An error occured, the argument following 'ip6 flow' is not a valid integer.      
    }
};
...
public:
    static Token* create_token(String argument_of_ip6_next_header_keyword, bool just_seen_a_not_keyword, Operator an_operator) {
        ...
        return NULL;    // An error occured, the argument following 'ip6 nxt' is not a valid integer.    
    }
...
public:
    static Token* create_token(String argument_of_ip6_ecn_keyword, bool just_seen_a_not_keyword, Operator an_operator) {
        ...
        return NULL;    // An error occured, the argument following 'ip6 ecn' is not a valid integer.       
    }
};
...
public:
    ...
    static Token* create_token(Vector<String> words_following_net, bool just_seen_a_not_keyword, Operator an_operator, bool& was_written_in_CIDR_style) {
        ...
        if ((slash_location = words_following_net[0].find_left('/')) != -1) {        // It is CIDR style
            ...
            click_chatter("words_following_net[0].substring(0,slash_location-1) = %s", words_following_net[0].substring(0,slash_location-1).c_str());
            click_chatter("words_following_net.size()-slash_location = %i", words_following_net[0].length()-slash_location-1);
